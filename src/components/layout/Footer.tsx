    ...
    <footer className="border-t border-line bg-base pt-10">
      <Container>
        <div className="flex flex-wrap justify-between gap-8 pb-7">
          <p className="max-w-[48ch] text-[15px] leading-[1.6] text-fg-muted">
          ...
          <div className="flex gap-12">
            {cols.map(([title, links]) => (
              <div key={title}>
                <p className="mb-3 text-[14px] font-semibold text-fg-strong">{title}</p>
                {links.map(([label, href]) => (
                  <a key={label} href={href} className="block min-h-[44px] py-2.5 text-[14px] text-fg-muted transition-colors hover:text-fg">
                    {label}
                  </a>
                ))}
              </div>
        ...
        <div className="flex flex-col gap-2 border-t border-line py-4 font-mono text-[11px] uppercase tracking-[0.12em] text-fg-muted sm:flex-row sm:items-center sm:justify-between">
